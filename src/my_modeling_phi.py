...
class LlamaYaRNScaledRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        if seq_len is None or seq_len > self.max_seq_len_cached:
            print("*****notice******")
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: PhiYarnConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                self.rotary_emb = LlamaDynamicNTKScalingRotaryEmbedding(
                    self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta
                )
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: PhiYarnConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: PhiYarnConfig):
...
class PhiYarnForCausalLM(LlamaPreTrainedModel):
