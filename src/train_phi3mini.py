...
import transformers
import deepspeed
from transformers import Trainer, GPT2ForQuestionAnswering, AutoConfig, AutoModel, AutoModelForCausalLM
from datasets import load_dataset

from my_configuration_phi3mini import Phi3MiniYarnConfig
from my_modeling_phi3mini import Phi3MiniYarnForCausalLM
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    sliding_window: int = field(default=2047, metadata={"help": "sliding window size"})
...
def train():
    ...
    config = AutoConfig.from_pretrained(model_args.model_name_or_path, cache_dir=training_args.cache_dir, sliding_window=training_args.sliding_window, attn_implementation="flash_attention_2", trust_remote_code=True) # flash_attention_2
    ...
    if training_args.rope_scaling_type is not None:
        print(f"Rope scaling type: {training_args.rope_scaling_type}")
    ...
    torch_dtype = config.torch_dtype if config.torch_dtype in [torch.float16, torch.bfloat16] else torch.bfloat16
    model = Phi3MiniYarnForCausalLM.from_pretrained(checkpoint, cache_dir=checkpoint, config=config, torch_dtype=torch_dtype)
    ...
    AutoConfig.register("phi3miniyarn", Phi3MiniYarnConfig)
    AutoModel.register(Phi3MiniYarnConfig, Phi3MiniYarnForCausalLM)
    AutoModelForCausalLM.register(Phi3MiniYarnConfig, Phi3MiniYarnForCausalLM)

    Phi3MiniYarnConfig.register_for_auto_class()
    Phi3MiniYarnForCausalLM.register_for_auto_class("AutoModelForCausalLM")
