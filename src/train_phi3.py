...
from my_modeling_phi3 import Phi3YarnForCausalLM
...
def train():
    ...
    config = AutoConfig.from_pretrained(model_args.model_name_or_path, cache_dir=training_args.cache_dir, attn_implementation="flash_attention_2", trust_remote_code=True) # flash_attention_2
    ...
    model = Phi3YarnForCausalLM.from_pretrained(checkpoint, cache_dir=checkpoint, config=config, use_flash_attention_2=True, trust_remote_code=True)
    ...
    AutoConfig.register("phi3yarn", Phi3YarnConfig)
    AutoModel.register(Phi3YarnConfig, Phi3YarnForCausalLM)
    AutoModelForCausalLM.register(Phi3YarnConfig, Phi3YarnForCausalLM)

    Phi3YarnConfig.register_for_auto_class()
    Phi3YarnForCausalLM.register_for_auto_class("AutoModelForCausalLM")
